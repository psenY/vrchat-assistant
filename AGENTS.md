...
- \`VRC_MONITOR_WS_SILENT_RECONNECT_MS\`：**WebSocket 应用层静默阈值（毫秒）**。ping/pong 只覆盖 TCP/WS 层——对端只要正常回 pong，即使一条应用消息都不推，心跳也永不判死（「连着但没事件」的半死连接，issue #247）。超过该阈值无任何 WS 消息即主动重连，并把 \`lastMessageAt\` / \`silentForSec\` 暴露在 \`/health.ws\`。**本 fork 的默认取 3 小时**（使用者实例实测合法静默最长 157.8 分钟）；**上游通用默认 30 分钟**（审核方实测最长 27.4 分钟）。调用时读取；空/非数字/非正数回落默认。
