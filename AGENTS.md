...
- `VRC_MONITOR_SELF_PRESENCE_OFFLINE_GRACE_SECONDS`：**出游戏确认窗口（秒）**（默认 `360`，范围 0-3600，调用时读取）。VRChat 在私人房之间切换 / 换图间隙会瞬时上报 `offline:offline`（实测 0.6-11.7s 居多，也有 349s / 352.5s 一档）；旧逻辑单次离线即判出游戏并立即写挂机文案，导致状态文字来回跳（issue #218）。窗口内判 **unknown**（消费方一律跳过、不写文案、不翻转判定态），满窗口才判 not_in_game。**边界（写实测数字，勿过度承诺）：本机实测覆盖 352.5s 档（该档不再误写）；不覆盖 7353.8s 档（超过窗口后仍会写挂机，属取舍）**；真出游戏后写文案会延后最多一个窗口；设 `0` 恢复旧行为。
- `VRC_MONITOR_ONLINE_INCLUDE_WEB`：**网页/移动端在线是否计入「在线好友数」**（默认 `1`=计入，`0`=只算游戏内有位置的好友）。只影响 MCP `get_online_friends` 的计数（状态文案的 `{total}`/`{webOnline}`/`{gameOnline}` 已是绝对值、不受本开关影响；旧别名 `{online}` 等同 `{total}`，仅作兼容保留）；好友列表的「网页在线」分组展示不受影响。网页在线语义：VRChat 转网页/App 在线时只发 `friend-active{platform:'web'}`（不发 friend-offline），REST 返回 `platform='web'`+`location='offline'`。
- `VRC_MONITOR_WS_SILENT_RECONNECT_MS`：**WebSocket 应用层静默阈值（毫秒）**。ping/pong 只覆盖 TCP/WS 层——对端只要正常回 pong，即使一条应用消息都不推，心跳也永不判死（「连着但没事件」的半死连接，issue #247）。超过该阈值无任何 WS 消息即主动重连，并把 `lastMessageAt` / `silentForSec` 暴露在 `/health.ws`。**本 fork 的默认取 3 小时**（使用者实例实测合法静默最长 157.8 分钟）；**上游通用默认 60 分钟**（判别带 (27.4min, 75min]，对数百好友实测尾部约 2.2x 余量）。调用时读取；空/非数字/非正数回落默认。
- `VRC_MONITOR_FRIEND_REFRESH_HOURS`：**好友资料周期刷新间隔**（默认 `6` 小时，最小 1）。服务纯 WS 驱动、无定期资料拉取——好友 `trust_level` 等资料字段只在 WS 事件到达时更新，陈旧后无自愈（2026-09-15 实测 XIAOFANG小芳已升 Trusted User、库内仍停 Known User）。本开关控制的周期任务**逐好友 `GET /users/{id}`**（`/auth/user/friends` 的 `offline` 参数是「仅在线/仅离线」二分过滤而非全量，故改按好友表 `last_seen DESC` 逐好友拉，顺带拿全 tags），**回写非空资料字段 + 以 `system_trust_*` tags 推导信任等级并记录变化事件**；**启动后 60 秒跑首轮**，之后每 `VRC_MONITOR_FRIEND_REFRESH_HOURS` 小时一轮；**每周期最多 `VRC_MONITOR_FRIEND_REFRESH_MAX` 个且取数窗口轮转** ⇒ 414 好友规模约 **9 个周期（≈54 小时）** 覆盖全部（**不是「部署后立即全量自愈」**，2026-09-22 按审核意见改正措辞）。每周期上限 `VRC_MONITOR_FRIEND_REFRESH_MAX`（默认 50，保护大好友数部署的 API 配额）。失败/非 200 仅 WARN 一行、限流走服务端。
...
- `VRC_MONITOR_FRIEND_REFRESH_HOURS`：**好友资料周期刷新间隔**（默认 `6` 小时，最小 1）。服务纯 WS 驱动、无定期资料拉取——好友 `trust_level` 等资料字段只在 WS 事件到达时更新，陈旧后无自愈（2026-09-15 实测 XIAOFANG小芳已升 Trusted User、库内仍停 Known User）。本开关控制的周期任务**逐好友 `GET /users/{id}`**（`/auth/user/friends` 的 `offline` 参数是「仅在线/仅离线」二分过滤而非全量，故改按好友表 `last_seen DESC` 逐好友拉，顺带拿全 tags），**回写非空资料字段 + 以 `system_trust_*` tags 推导信任等级并记录变化事件**；**启动后 60 秒跑首轮**，之后每 `VRC_MONITOR_FRIEND_REFRESH_HOURS` 小时一轮；**每周期最多 `VRC_MONITOR_FRIEND_REFRESH_MAX` 个且取数窗口轮转** ⇒ 414 好友规模约 **9 个周期（≈54 小时）** 覆盖全部（**不是「部署后立即全量自愈」**，2026-09-22 按审核意见改正措辞）。每周期上限 `VRC_MONITOR_FRIEND_REFRESH_MAX`（默认 50，保护大好友数部署的 API 配额）。失败/非 200 仅 WARN 一行、限流走服务端。
- `VRC_MONITOR_WS_SILENT_RECONNECT_MS`：**WebSocket 应用层静默阈值（毫秒，默认 3600000 = 60 分钟；判别带 (27.4min, 75min]，对数百好友实测尾部约 2.2x 余量）**。ping/pong 只覆盖 TCP/WS 层——对端只要正常回 pong，即使一条应用消息都不推，心跳也永不判死（「连着但没事件」的半死连接，issue #247）；超过该阈值无任何 WS 消息即主动重连，并把 `lastMessageAt` / `silentForSec` 暴露在 `/health.ws`。调用时读取；空/非数字/非正数回落默认。**取值注意**：实测合法静默最长可达 27 分钟（数百好友）与 157.8 分钟（24 好友），故不要设得过小，否则安静时段会误重连。
