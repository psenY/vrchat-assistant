...
- `VRC_MONITOR_ONLINE_INCLUDE_WEB`：**网页/移动端在线是否计入「在线好友数」**（默认 `1`=计入，`0`=只算游戏内有位置的好友；**调用时读取**）。两处口径统一走 `core/online-count-policy.js`，影响面：
  ① **MCP `get_online_friends` 的 `online` 计数**（该工具自己发 `GET /auth/user/friends?offline=false`，现已复用同一策略）；
  ② 服务自身的在线数 `friendState.getOnlineCount()` ⇒ 动态状态模板 `{online}`、dashboard 的 `onlineNow`、`get_dynamic_status` / `get_server_status` 返回值里的 `onlineNow`。
  好友列表的「网页在线」分组**展示**不受影响。语义背景：VRChat 转网页/App 在线时只发 `friend-active{platform:'web'}`（不发 friend-offline），REST 返回 `platform='web'` + `location='offline'`。
