...
- `VRC_MONITOR_ONLINE_INCLUDE_WEB`：**网页/移动端在线是否计入「在线好友数」**（默认 `1`=计入，`0`=只算游戏内有位置的好友）。只影响 MCP `get_online_friends` 的计数（状态文案的 `{online}`/`{total}` 已是绝对值、不受本开关影响）；好友列表的「网页在线」分组展示不受影响。网页在线语义：VRChat 转网页/App 在线时只发 `friend-active{platform:'web'}`（不发 friend-offline），REST 返回 `platform='web'`+`location='offline'`。
