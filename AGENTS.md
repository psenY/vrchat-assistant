...
- `VRC_MONITOR_PRESENCE_STATUS_OFFLINE_CONFIRM`：**presence-status 的出游戏去抖次数**（默认 `2`，范围 1-10）。VRChat 在私人房之间切换 / 换图间隙会瞬时上报 `offline:offline`（有毫秒级与分钟级两种形态）；旧逻辑把「最近一条位置是 offline」直接判为出游戏并立即写挂机文案，导致状态文字来回跳（issue #218）。设为 N 后只有**连续 N 次轮询**都判为「不在游戏」才写挂机文案；中间出现 in_game/unknown 会清零计数。设为 `1` 即恢复旧行为。
