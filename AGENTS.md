...
- `VRC_MONITOR_FRIEND_REFRESH_HOURS`：**好友资料周期刷新间隔**（默认 `6` 小时，最小 1）。服务纯 WS 驱动、无定期资料拉取——好友 `trust_level` 等资料字段只在 WS 事件到达时更新，陈旧后无自愈（2026-09-15 实测 XIAOFANG小芳已升 Trusted User、库内仍停 Known User）。本开关控制的周期任务**逐好友 `GET /users/{id}`**（`/auth/user/friends` 实测硬性只返回 20 个、覆盖不全，故按好友表 `last_seen DESC` 逐个拉），**回写非空资料字段 + 以 `system_trust_*` tags 推导信任等级并记录变化事件**；**启动后 60 秒跑首轮**（部署后立即自愈存量陈旧等级）。每周期上限 `VRC_MONITOR_FRIEND_REFRESH_MAX`（默认 50，保护大好友数部署的 API 配额）。失败/非 200 仅 WARN 一行、限流走服务端。
